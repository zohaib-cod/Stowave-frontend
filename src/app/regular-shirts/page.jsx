...
export default function RegularShirtsPage() {
  ...
  useEffect(() => {
    ...
    // Load favorites (keep as IDs)
    const savedFavs = JSON.parse(localStorage.getItem("favorites")) || [];
    setFavorites(savedFavs.map((item) => item.id));
  }, [BASE_URL]);

  // Toggle favorite (same logic as OversizeShirts)
  const toggleFavorite = (product) => {
    let existingFav = JSON.parse(localStorage.getItem("favorites")) || [];
    const exists = existingFav.find((item) => item.id === product.id);
  ...
  // Add to cart (uses normalized selectedProduct shape)
  const handleAddToCart = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 md:py-12 text-black dark:text-white transition-colors duration-300">
      <h2 className="text-2xl md:text-3xl font-bold mb-4 text-center md:text-left text-black dark:text-white">
        Regular-Shirts
      </h2>

      <p className="text-gray-500 mb-6 md:mb-8 text-sm md:text-base text-center md:text-left dark:text-gray-300">
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="w-full h-48 sm:h-56 md:h-64 lg:h-72 overflow-hidden rounded-none bg-gray-200 dark:bg-white/10 animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
      ...
      {/* Modal (same UI/logic as OversizeShirts) */}
      {selectedProduct && (
